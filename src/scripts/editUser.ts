...
dispatcher.authenticate().then(async () => {
    ...
    prompt([
        ...
        const { username, selection } = results as { username: string; selection: 'databases' | 'permissions'; };
        ...
        if (selection === 'permissions') {
            ...
            prompt([
                ...
                const { action } = value as { action: 'add' | 'remove' | 'list'; };
                ...
                switch (action) {
                    ...
                    case 'remove': {
                        ...
                        if (!user.databases.length) throw new Error(`This user does not have access to any databases`);
